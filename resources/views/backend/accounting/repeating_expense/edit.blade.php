...
<div class="row">
	<div class="col-12">
		<div class="card">
			<span class="d-none panel-title">{{ _lang('Update Repeating Expense') }}</span>

			<div class="card-body">
				<form method="post" class="validate" autocomplete="off" action="{{ action('RepeatingExpenseController@update', $id) }}" enctype="multipart/form-data">
					{{ csrf_field()}}
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<select class="form-control select2" name="status" required>
							   ...
							   <option value="1" {{ $transaction->status == 1 ? "selected" : "" }}>{{ _lang('Completed') }}</option>
							</select>
						  </div>
						</div>
